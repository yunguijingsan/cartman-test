...
function postProxy(req, res, method) {
	var data = "";
	var proxyPath = req.url;
	if (is(req, [ 'json' ])) {
		data = req.body;//getJsonParam(req.body);
	} else {
...
function proxy(req, res, opt, data, isPost) {
	console.log(new Date().getTime() + "    " + req.url + " request start"
			+ "    " + opt);
	var req_proxy = http.request(
			opt,
					...
					body += data;
						...
						function() {
							console.log(new Date().getTime() + "    "
									+ opt.method + "    " + opt.host + ":"
									+ opt.port + opt.path + " request finish");
			...
			'error',
			function(data) {
				console.log(new Date().getTime() + "    " + opt.method + "    "
						+ opt.host + ":" + opt.port + opt.path
						+ " request finish");
